...
def make_request_with_retries(url, session):
    ...
    for i in range(MAX_RETRIES):
        try:
            ...
            if response.status_code in (502, 503, 504):  # Retry on server errors
...
def generate_redirect_graph(results, domain):
    if not HAS_GRAPHVIZ:
        logging.error("Cannot generate redirect graph: graphviz library not installed.")
        return

    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"redirect_graph_{domain}_{timestamp}.png"
    ...
    try:
        ...
        logging.info(f"Redirect graph saved as {filename}")
    except Exception as e:
        logging.error(f"Failed to generate redirect graph: {e}")
...
def main():
    ...
    display_colored_art(ASCII_ART, args.show_art, scale=1)  # Scales 2x the ascii art size
    print(CREATED_BY)
    print(VERSION)
    ...
    if results and HAS_GRAPHVIZ:
        while True:
            try:
                generate_graph = input("\nGenerate a PNG graph of the redirect chain? (y/n): ").strip().lower()
                if generate_graph in ['y', 'n']:
                    break
                ...
                break
        if generate_graph == 'y':
            generate_redirect_graph(results, domain)
    elif not HAS_GRAPHVIZ:
        logging.warning("Graph generation skipped: graphviz library not installed.")
